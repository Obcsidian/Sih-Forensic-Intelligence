...
export default function PreviewPane({
  ...
  if (!evidence) {
    return (
      <div className="glass-panel flex h-56 shrink-0 items-center justify-center rounded-t-2xl m-2 text-xs text-gray-500">
        <div className="text-center">
          <div className="flex h-12 w-12 mx-auto items-center justify-center rounded-lg bg-cyan-400/15 text-cyan-300 mb-2">⧉</div>
  ...
  return (
    <div className="glass-panel rounded-t-xl border-b border-border bg-white/[0.04] flex h-64 shrink-0 flex-col">
      <div className="flex gap-1 rounded-xl border-b border-border/60 px-2 py-1">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`rounded-full px-2.5 py-1 text-[11px] transition-colors ${tab === t.id ? "bg-accent/20 text-accent" : "text-gray-400 hover:bg-panel3 transition-all"} ${tab === t.id ? "shadow-[0_2px_4px_rgba(34,211,238,0.2)]" : ""}`}
          >
            {t.label}
          </button>
        ))}
